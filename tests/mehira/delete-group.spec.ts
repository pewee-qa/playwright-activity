...
test("User creates a group", async ({ page }) => {
  ...
  //When user wants to delete a group
  ...
  await page
    .locator(':nth-match([data-testid="table-menu-button"], 1)')
    .click();
  await page
    .locator('[data-testid="table-menu-view"]', { hasText: "Delete" })
    .click();
  await page.locator("button", { hasText: "Delete" }).click();

  //Then user successfully deletes the group
  const deleteMessage = data.Groupremoved;

  await expect(page.locator("#toast-2")).toContainText(deleteMessage);
});
