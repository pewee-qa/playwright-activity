...
test("User creates a group", async ({ page }) => {
  ...
  //When user wants to edit a group
  ...
  //Then user is redirected to the Group details modal
  await page
    .locator(':nth-match([data-testid="table-menu-button"], 1)')
    .click();
  await page
    .locator('[data-testid="table-menu-view"]', { hasText: "View" })
    .click();
  await page.locator("button", { hasText: "Edit" }).click();

  await page.getByRole("dialog", { name: "Group details" }).isVisible;
});
